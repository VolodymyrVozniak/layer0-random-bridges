...
from .settings import SETTING_BITCOINRIDGE_LIST


def bitcoin_bridge(name, private_key, from_chain, to_chain, max_bridge, max_gas, max_value):
    log_name = f'BRIDGE BTC.b {from_chain} to {to_chain}'

    from_data = search_setting_data(chain=from_chain, list=SETTING_BITCOINRIDGE_LIST)
    if len(from_data) == 0:
    ...
    to_data = search_setting_data(chain=to_chain, list=SETTING_BITCOINRIDGE_LIST)
    if len(to_data) == 0:
